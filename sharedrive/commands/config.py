...
ACTIVE_DESCRIPTOR = Path(".sharedrive/descriptor")


def active_descriptor() -> Path | None:
    if not ACTIVE_DESCRIPTOR.exists():
        return None
...
def set_active_descriptor(path: Path) -> Path:
    if not path.is_file():
        raise ValueError(f"Descriptor '{path}' does not exist.")
