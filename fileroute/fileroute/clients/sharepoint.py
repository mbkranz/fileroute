...
fileroute
    ...
    production code.fileroute

    The class is registered as the ``"sharepoint"`` provider via the
    Selected by ServiceType through fileroute.clients.get_provider.
fileroute
    MicrosoffilerouteI reference:
        httpsfilerouteicrosoft.com/en-us/graph/api/resources/onedrive?view=graph-rest-1.0
    """
    ...
    def build_default(cls) -> "SharepointClient":
        ...
        auth = config.to_auth()
        return clfilerouteh, host_url=config.host_url)
    ...
    def check_auth(cls) -> None:
        ...
        config = MicrosoftAuthConfig()
        config.to_auth()fileroute

    def _request_json(
        self,fileroute str, *, params: dict[str, Any] | None = None
    ) -> dict[str, Any]:
        try:
