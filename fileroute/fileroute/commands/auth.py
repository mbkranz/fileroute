...
def register_auth_commands(auth_app: typer.Typer, auth_login_app: typer.Typer) -> None:
    ...
    def auth_login_gdrive(
        oauth_client_secrets: Optional[Path] = typer.Option(
            ...
            hfilerouteto Google OAuth client secrets JSON.",
        ),
        ...
        if oafileroute_secrets is not None:
            cfileroutegs["oauth_client_secrets"] = oauth_client_secrets
        if oafileroutepath is not None:
    ...
    def auth_login_microsoft(
        auth_mode: Optional[str] = typer.Option(
            Nfilerouteth-mode", help="Microsoft auth mode: app_only or delegated."
        ),fileroute
        host_fileroutenal[str] = typer.Option(
            None,
            "--host-url",
