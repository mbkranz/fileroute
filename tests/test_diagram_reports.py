...
def test_html_escapes_metadata_and_rejects_executable_links(tmp_path):
    ...
    ids = re.findall('\\bid="([^"]+)"', report)
    assert len(ids) == len(set(ids))
    for anchor in re.findall('href="#([^"]+)"', report):
...
def test_markdown_companion_and_dictionary_links(tmp_path):
    ...
    anchors = set(re.findall('<a id="([^"]+)"', text))
    assert set(re.findall("\\]\\(#([^)]*)\\)", text)) <= anchors
    ...
    assert all(
        (
            group.find("{http://www.w3.org/2000/svg}title") is not None
            for group in groups
        )
    )
