...
* `--name, --select TEXT`: Entity or location by name, dot-path, JSON Pointer, or exact JSONPath. Defaults to the descriptor root. Use 'fileroute list' to see exact JSONPaths.
...
* `--select TEXT`: Show one entity, location, or catalog subtree by name, JSON Pointer, or exact JSONPath. Use 'fileroute list' to see exact JSONPaths.
* `--help`: Show this message and exit.
...
* `--descriptor PATH`: Descriptor file path. Defaults to the saved descriptor or the first standard descriptor path.
* `--parent TEXT`: Catalog parent by name, dot-path, JSON Pointer, or exact JSONPath; defaults to root. Use 'fileroute list' to see exact JSONPaths.
* `--help`: Show this message and exit.
